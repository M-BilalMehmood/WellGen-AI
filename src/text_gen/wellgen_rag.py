...
class WellGenRAG:
    ...
    def validate_diet_plan(self, diet_plan, user_data):
        """Cross-validate the generated diet plan using Groq AI and apply minor improvements."""
...
OVERALL ASSESSMENT: [1-2 sentence summary]"""
        ...
        try:
            ...
            has_suggestions = "suggestions:" in result_lower and "suggestions: none" not in result_lower
            
            # Apply minor improvements if there are suggestions
            updated_plan = diet_plan
            if has_suggestions:
                # Silently apply improvements
                updated_plan = self._apply_minor_improvements(diet_plan, validation_result, user_data)
            ...
            if "validation result: pass" in result_lower:
                ...
                return True, validation_result, updated_plan
                ...
                print(validation_result.split("ISSUES FOUND:")[1].split("SUGGESTIONS:")[0].strip())
                return False, validation_result, updated_plan
                
        except Exception as e:
            print(f"⚠️  Validation error: {e}")
            print("Proceeding with plan (validation unavailable)")
            return True, "Validation error - proceeding", diet_plan
    
    def _apply_minor_improvements(self, diet_plan, validation_feedback, user_data):
        """Apply minor improvements to the diet plan based on validation feedback."""
        try:
            improvement_prompt = f"""You are a nutrition expert. Make MINOR improvements to this diet plan based on the validation feedback.
...
INSTRUCTIONS:
1. Keep the overall structure and most meals the same
2. Make only SMALL, TARGETED improvements based on the suggestions
3. Do NOT regenerate the entire plan - just tweak specific parts
4. Examples of minor improvements:
   - Adjust portion sizes slightly for better calorie balance
   - Swap one ingredient for better nutrition (e.g., white rice → brown rice)
   - Add a missing nutrient source (e.g., add nuts for healthy fats)
   - Improve variety by changing 1-2 repetitive meals
5. Maintain the same format and structure
7. CRITICAL: Return ONLY the updated diet plan. Do NOT include any introductory text, explanations, or list of changes. Start directly with the plan.

Return the UPDATED diet plan with minor improvements applied."""

            response = self.groq_client.chat.completions.create(
                model="llama-3.3-70b-versatile",
                messages=[
                    {"role": "system", "content": "You are a nutrition expert making minor refinements to diet plans."},
            ...
            # print("✓ Minor improvements applied")
